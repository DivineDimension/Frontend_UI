...
const MyNFTCopy2 = (props) => {
    ...
    useEffect(() => {
        document.title = "DivineDimension | MyNFT"
    }, [])    
    ...
    const dbcallalgoActivity=async()=>{        
        ...
        //firebase.auth().signInAnonymously().then((response)=>{              
          firebase.database().ref("EAptosactivitytable").child(getalgo).on("value", (data) => {
            if (data) {
              data.forEach((d) => {                
                let value=d.val();                
                req.push(            
    ...
    const dbcallalgos=async()=>{        
        ...
        let getalgo=location.state.allData.walletAddress;              
        //firebase.auth().signInAnonymously().then((response)=>{      
            firebase.database().ref("EAptosNFTNS").child(getalgo).on("value", (data) => {
            if (data) {
              data.forEach((d) => {                
                let value=d.val();
                req.push(            
            ...
            firebase.database().ref("EAptosNFTRs").child(getalgo).on("value", (data) => {
                if (data) {
                  data.forEach((d) => {                
                    let value=d.val();
                    req.push(            
            ...
            firebase.database().ref("EAptosNFTAS").child(getalgo).on("value", (data) => {
                if (data) {
                    data.forEach((d) => {                
                    let value=d.val();
                    req.push(            
    ...
    const dbcallProfile=async()=>{            
        ...
        //firebase.auth().signInAnonymously().then((response)=>{      
        firebase.database().ref("EPuserprofile").child(location.state.allData.walletAddress).on("value", (data) => {          
          if (data) {                      
              r.push({
                Bio:data.val().Bio,
    ...
    const dbcallalgo=async()=>{        
        ...
        let getalgo=location.state.allData.walletAddress
        //firebase.auth().signInAnonymously().then((response)=>{      
          firebase.database().ref("EAptosNFTN").child(getalgo).on("value", (data) => {
            if (data) {
              data.forEach((d) => {                
                let value=d.val();
                req.push(            
            ...
            firebase.database().ref("EAptosNFTR").child(getalgo).on("value", (data) => {
                if (data) {
                  data.forEach((d) => {                
                    let value=d.val();
                    req.push(            
                ...
                firebase.database().ref("EAptosNFTA").child(getalgo).on("value", (data) => {
                    if (data) {
                      data.forEach((d) => {                
                        let value=d.val();
                        req.push(            
    ...
    const dbcallalgoowned=async()=>{
        ...
        let getalgo=location.state.allData.walletAddress
        //firebase.auth().signInAnonymously().then((response)=>{      
          firebase.database().ref("EAptosNFTNBuy").child(getalgo).on("value", (data) => {
            if (data) {
              data.forEach((d) => {                
                let value=d.val();
                req.push(            
            ...
            firebase.database().ref("EAptosNFTRBuy").child(getalgo).on("value", (data) => {
                if (data) {
                  data.forEach((d) => {                
                    let value=d.val();
                    req.push(            
                ...
                firebase.database().ref("EAptosNFTABuy").child(getalgo).on("value", (data) => {
                    if (data) {
                      data.forEach((d) => {                
                        let value=d.val();
                        req.push(            
                             ...
                             <div className='d-flex flex-wrap flex-lg-nowrap align-items-center create-art'>                             
                                 <div className=''>
                                     {getIProfile === "" || getIProfile === null || getIProfile === undefined || getIProfile[0] === "" || getIProfile[0] === null || getIProfile[0] === undefined?(
                                        ...
                                        <a  href={"https://testnet.algoexplorer.io/address/"+location.state.allData.walletAddress} target="_blank" rel="noreferer noreferrer">
                                ...
                                {filterdata().map((x, index) => {   
                                    ...
                                    return( 
                                        ...
                                        <Card className='card-dash p-3 d-block border-0'>
                                            ...
                                            <h6 className='mb-2'>Price</h6><h4 className='d-flex mb-3 align-items-center'><img src={getIProfile[0].Imageurl} alt="logo" className='me-2 avatar-pic' /> {x.NFTPrice/100000000}</h4>                                             
                                        </Card>
                                        </Col>
                                    )
                                })}                                                                
                                ...
                                return(                             
                                    ...
                                    <div className='activity-item d-flex align-items-center mb-3'>        
                                            <div className="activity-content">                                        
                                                ...
                                                <p style={{cursor: 'pointer'}} onClick={() => window.open(`https://explorer.aptoslabs.com/account/${x.ownerAddress}`)}> {x.ownerAddress}</p>                
                                                {x.NFTDescription === "" || x.NFTDescription === null || x.NFTDescription === undefined ?(<>
                                                </>):(<>
                                                    <p style={{cursor: 'pointer'}} onClick={() => window.open(`https://explorer.aptoslabs.com/account/${x.NFTDescription}`)}>  {x.NFTDescription}</p>
                                                </>)}
