...
const ViewSpecialNFT = () => {
    ...
    const dbcallalgoActivity=async()=>{        
        ...
        fireDb.database().ref("EPolygonNFTNLimit").child(getalgo).on("value", (data) => {
            if (data) {
              data.forEach((d) => {                
                let value=d.val();                
                req.push(            
    ...
    const filterdata5=()=>{
        ...
        if(getrecent === "Low to High"){
          let data=getImgreffalgoActivity.reverse().sort((a,b)=>{ return parseFloat(a.NFTPrice/100000000) - parseFloat(b.NFTPrice/100000000)})          
          return data;
        }
        if(getrecent ===  "High to Low"){
          let data=getImgreffalgoActivity.reverse().sort((a,b)=>{ return parseFloat(b.NFTPrice/100000000) - parseFloat(a.NFTPrice/100000000)})          
          return data;
        }                
                                ...
                                return(                                     
                                    ...
                                    <div className='activity-item d-flex align-items-center mb-3'>        
                                            <div className="activity-content">                                                                                                
                                                {x.ownerAddress === "" || x.ownerAddress === null || x.ownerAddress === undefined ?(<>
                                                </>):(<>
                                                    <p style={{cursor: 'pointer'}} onClick={() => window.open(`https://mumbai.polygonscan.com/address/${x.ownerAddress}`)}>  {x.ownerAddress}</p>
                                                </>)}                                            
                                                {x.EscrowAddress === "" || x.EscrowAddress === null || x.EscrowAddress === undefined ?(<>
                                                </>):(<>
                                                    <p style={{cursor: 'pointer'}} onClick={() => window.open(`https://mumbai.polygonscan.com/address/${x.EscrowAddress}`)}>  {x.EscrowAddress}</p>
                                                </>)}
                                                <p className="heading mb-0">{x.NFTName}</p>
                                                <p className="heading mb-0">{x.userSymbol}</p>
