...
const OwnedTab =({x})=>{
    ...
    const setpricedb=async(b)=>{        
        ...
        else if(localStorage.getItem('EAWalletAddress') === x.ownerAddress){         
        ...
        try {                              
                ...
                await getaaaa.methods.setTokenPrice(b.Assetid,web3.utils.toBN(amountmul),b.SocialLink).send({
                    from:localStorage.getItem('EAWalletAddress'),              
                    gas: 210000,
                    //gasPrice: '20000000000'
                })
        ...
        //fireDb.auth().signInAnonymously().then((responses)=>{                     
          let dateset=new Date().toDateString();      
            fireDb.database().ref(`EPolygonNFTRS/${localStorage.getItem('EAWalletAddress')}`).child(b.keyId).set({
                ...
                fireDb.database().ref(`EPolygonNFTRBuy/${localStorage.getItem('EAWalletAddress')}`).child(b.keyId).remove();
